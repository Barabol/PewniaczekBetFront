        ...
        <div className="lg:col-span-1">
          <div className="bg-gradient-to-br from-green-600 to-green-700 text-white rounded-lg shadow-lg p-6 mb-6">
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              ...
              <div>
                <div className="opacity-80">{t('Stan konta')}</div>
          ...
          <div className="bg-card rounded-lg shadow-md border border-border overflow-hidden">
            <div className="flex border-b border-border">
              <button
                ...
                className={`flex-1 py-3 px-4 transition cursor-pointer ${
                  activeTab === 'deposit'
            ...
            <div className="p-6">
              <div className="mb-4">
                <label className="block text-sm mb-2">{t('Kwota (PLN)')}</label>
                <input
                  type="number"
                  value={amount}
              ...
              >
                {loading ? t('Przetwarzanie...') : activeTab === 'deposit' ? t('Wpłać środki') : t('Wypłać środki')}
              </button>
        ...
        <div className="lg:col-span-2">
          <div className="bg-card rounded-lg shadow-md border border-border">
            <div className="p-6 border-b border-border">
              <h3>{t('Podsumowanie')}</h3>
            </div>

            <div className="p-6 space-y-4">
              <div className="flex items-center justify-between p-4 bg-muted rounded-lg">
                <div>
                  <div className="font-medium">{t('Saldo główne')}</div>
                  <div className="text-sm text-muted-foreground">{t('Dostępne do obstawiania')}</div>
                </div>
                <div className="text-xl font-bold text-green-600">{user?.balance?.toFixed(2) || '0.00'} PLN</div>
              </div>
              <div className="flex items-center justify-between p-4 bg-muted rounded-lg">
                <div>
                  <div className="font-medium">{t('Freebet')}</div>
                  <div className="text-sm text-muted-foreground">{t('Środki bonusowe')}</div>
                </div>
          ...
          <div className="bg-card rounded-lg shadow-md border border-border mt-6">
            <div className="p-6">
              <button
                ...
                className="w-full flex items-center justify-center gap-2 p-3 bg-green-500/10 text-green-700 dark:text-green-300 rounded-lg hover:bg-green-500/20 transition cursor-pointer"
              >
                <ExternalLink className="w-5 h-5" />
                {t('Odśwież stan konta')}
              </button>
            </div>
          </div>
          ...
          <div className="bg-card rounded-lg shadow-md border border-border mt-6 overflow-hidden">
            <div className="p-6 border-b border-border">
              <h3>{t('Historia transakcji')}</h3>
            </div>

            {loadingPayments ? (
              <div className="p-6 text-center text-muted-foreground text-sm">{t('Ładowanie transakcji...')}</div>
            ) : payments.length === 0 ? (
              <div className="p-6 text-center text-muted-foreground text-sm">{t('Brak historii transakcji')}</div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm border-collapse">
                  <thead>
                    <tr className="border-b border-border bg-muted/40 text-muted-foreground font-medium">
                      <th className="py-3 px-4">{t('Opis')}</th>
                      <th className="py-3 px-4">{t('Kwota')}</th>
                      <th className="py-3 px-4">{t('Data')}</th>
